...
RSpec.describe Basechurch::User, :type => :model do
  let(:user) { create(:user) }

  describe '#session_api_key' do
    context 'when user does not have an access token yet' do
      it 'creates a new one' do
        expect { user.session_api_key }.to change { Basechurch::ApiKey.count }.by(1)
      end
    end
    ...
    context 'when user already has access token' do
      ...
      it 'reuses existing token' do
        expect { api_key }.to_not change { Basechurch::ApiKey.count }
